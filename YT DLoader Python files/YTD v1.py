...
import socket    # Fallback for timeout
...
class YTDLoader(ctk.CTk):
    def __init__(self):
        ...
        # spinner for fetching formats (beside resolution box)
        self._spinner_running = False
        self._spinner_after_id = None
        self._spinner_chars = ["⠋","⠙","⠸","⠴","⠦","⠇","⠋","⠙","⠸","⠴","⠦","⠇"]
        self._spinner_index = 0

        # independent spinner shown in status_label while downloading
    ...
    def _start_auto_retry(self, action_func):
        ...
        def check_and_retry():
            if self._check_internet():
                ...
                action_func()  # Auto-fetch or auto-download
            else:
                # Retry in 5 sec
    ...
    def _build_ui(self):
        ...
        main_right.grid_rowconfigure(1, weight=1)

        log_header_frame = ctk.CTkFrame(main_right, fg_color="transparent")
        log_header_frame.grid(row=0, column=0, sticky="ew", pady=(10, 5))
    ...
    def _dl_spinner_step(self):
        ...
        # remove any trailing spinner token if present
        if txt and txt.strip():
            ...
            txt = ""
        # ensure there's a space before spinner
        txt = txt.rstrip()
        if txt:
    ...
    def _on_download(self):
        ...
        if not self._check_internet():
            self._show_no_internet_popup()
            self._start_auto_retry(self._on_download)  # Auto-retry this exact download
            return
        ...
        ydl_opts = {
            ...
            "quiet": True,
            "no_warnings": True,
            "retries": 5,  # Retry up to 10 times on SSL/network errors
            "fragment_retries": 10,  # Retry failed video chunks
            "sleep_interval": 1,    # wait 1 sec between retries
    ...
    def _download_worker(self, url, ydl_opts):
        try:
            ...
            if not self._check_internet() or any(x in err_msg for x in ["ssl", "decryption", "timeout", "connection", "network", "http"]):
                # No internet or poor: popup + auto-retry
    ...
    def _progress_hook(self, d):
        try:
            ...
            if st == "downloading":
                ...
                if not self._check_internet():
                    self.queue.put(("no_internet_download", None))  # Trigger retry
                else:
                    self.queue.put(("log", "Network retrying chunk..."))
                return  # ← Stops false "Invalid URL"

        except Exception:
            raise  # Let real errors go to worker
        ...
        try:
            while True:
                ...
                if kind == "formats_ready":
                    ...
                    # keep dl-spinner independent: do NOT update spinner here
                    if pct is None:
                        ...
                        # update label text but don't touch spinner characters here
                        self.status_label.configure(text=status)
                    ...
                    self._cancel_requested = False
                    self.progress.set(0)
                    self.retry_count = 0  # Reset for next try

                    # always stop dl spinner on error/cancel
                    self._stop_dl_spinner()
                    self.download_btn.configure(state="normal")
                    self.cancel_btn.configure(state="disabled")
                    self._cancel_requested = False
